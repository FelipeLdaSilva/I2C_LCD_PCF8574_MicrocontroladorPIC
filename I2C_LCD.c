...
void LCD_Write_Number(unsigned short length, unsigned int number)
{
  ...
  {
	LCD_Write_Char(((number/div)%10)+'0');
	div = div/10;
  }
}
void LCD_Create_Char(unsigned short CGRAM_Position, unsigned char Custom_Char[])
{
  unsigned short CGRAM_Address_Array[8]={0x40,0x48,0x50,0x58,0x60,0x68,0x70,0x78};
  LCD_CMD((char)CGRAM_Address_Array[CGRAM_Position]);
  LCD_CMD(0x00);
